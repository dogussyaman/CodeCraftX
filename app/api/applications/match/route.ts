...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const js of jobSkills || []) {
      const skillName = (js.skills as { name?: string })?.name
    ...
    const developerSkillNames = new Set(
      (devSkills || []).map((ds) => (ds.skills as { name?: string })?.name?.toLowerCase()).filter(Boolean)
    )
    ...
    if (requiredSkills.length > 0) {
      ...
      const optionalBonus =
        optionalSkills.length > 0
          ...
          : 0
      skillScore = clampScore((requiredMatchRate + optionalBonus) * 100)
    ...
    if (jobLevel && cvId) {
      ...
      const jOrder = levelOrder[jobLevel] ?? 0
      const cvOrder = cvSeniority
        ? levelOrder[cvSeniority] ?? 0
    ...
    const { data: profile } = await supabase.from("profiles").select("bio, title").eq("id", developerId).single()
    if (profile?.bio) cvParts.push(profile.bio)
    if (profile?.title) cvParts.push(profile.title)

    if (cvId) {
      const { data: cvRow } = await supabase.from("cvs").select("raw_text").eq("id", cvId).single()
    ...
    if (educations && educations.length > 0) {
      ...
      educationScore = fieldRelevant ? 100 : 50
      if (fieldRelevant) positiveFactor.push("Eğitim alanı pozisyonla ilgili")
      educationMatch = {
    ...
    const finalScore = Math.round(
      skillScore * 0.4 + experienceScore * 0.25 + textScore * 0.25 + educationScore * 0.1
    )

    let matchReason = ""
    if (finalScore >= 80) matchReason = "Aday, pozisyon gereksinimleriyle yüksek oranda uyumlu. "
    else if (finalScore >= 60) matchReason = "Aday, pozisyon için uygun görünüyor. "
    else if (finalScore >= 40) matchReason = "Aday, bazı gereksinimleri karşılıyor ancak eksiklikler var. "
    else matchReason = "Aday, pozisyon gereksinimleriyle düşük uyumlu. "
    if (matchingSkills.length > 0) matchReason += `${matchingSkills.length} yetenek eşleşmesi bulundu. `
    if (experienceAnalysis?.level_match) matchReason += "Deneyim seviyesi uygun. "
    ...
    const { error: updateError } = await supabase
      .from("applications")
      .update({ match_score: finalScore, match_reason: matchReason, match_details: matchDetails })
      .eq("id", applicationId)
