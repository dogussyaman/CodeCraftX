...
const HERO_HINT = "CV analizi, akıllı eşleşme ve hızlı işe alım tek platformda."
const HERO_STACK = [
  {
    id: 0,
    z: "z-10",
      ...
      <div className="container relative mx-auto px-4">
        <div className="mx-auto max-w-6xl">
          <div className="pointer-events-none absolute right-0 top-8 hidden h-64 w-80 xl:block">
            {FLOATING_BADGES.map((item, idx) => (
              <motion.div
                ...
                animate={{ opacity: [0.55, 0.95, 0.55], y: [0, -10, 0], x: [0, idx % 2 === 0 ? 6 : -6, 0] }}
                transition={{
                  duration: item.dur,
                  delay: item.delay,
                  repeat: Infinity,
          ...
          <motion.h1
            ...
            className="max-w-3xl text-balance text-4xl font-semibold leading-tight text-foreground dark:text-white sm:text-5xl md:text-6xl"
          >
            Büyük fikrinin hak ettiği
            <span className="block font-semibold text-[var(--brand-ember)] dark:text-[var(--brand-ember)]">
              modern işe alım tasarımı
            </span>
          </motion.h1>
          ...
          >
            <Button
              asChild
              size="lg"
              className="h-12 rounded-full border border-accent-500/50 bg-gradient-to-r from-accent-500 to-accent-400 px-6 text-sm font-semibold text-white shadow-lg shadow-accent-500/25 transition hover:from-accent-600 hover:to-accent-500"
            >
          ...
          >
            ...
            >
              {HERO_STACK.map((layer) => (
                ...
                >
                  <div className="relative h-full overflow-hidden rounded-2xl border border-accent-500/20 bg-white/70 p-2 shadow-[0_30px_80px_rgba(124,45,18,0.22)] backdrop-blur dark:border-white/15 dark:bg-zinc-950/70 dark:shadow-[0_30px_80px_rgba(0,0,0,0.55)]">
                    <Image
                      src="/ss1.png"
                      ...
                      height={1080}
                      priority={layer.id === 0}
                      className="h-full w-full rounded-xl border border-accent-500/15 object-cover dark:border-white/10"
                    />
                  </div>
                </motion.div>
              ))}
